...
def prepare_turtle_canvas():
    ...
    n = 1
    while True:
        draw_curve_5_points(points[(n - 1) % 5], points[(n) % 5], points[(n + 1) % 5], points[(n + 2) % 5],
                            points[(n + 3) % 5])
...
def draw_curve_5_points(p1, p2, p3, p4, p5):
    ...
    for i in range(0, 50, 2):
        t = i / 100
        x = (2*t**2-3*t+1)*p5[0]+(-4*t**2+4*t)*p1[0]+(2*t**2-t)*p2[0]
        y = (2*t**2-3*t+1)*p5[1]+(-4*t**2+4*t)*p1[1]+(2*t**2-t)*p2[1]
        draw_point((x, y))
    draw_point(p2)
